    ...
    namespace Core
    {
        ...
        namespace UploadDownloadFile
        {
            ...
            public class UploadFile
            {
                ...
                public UploadFile(FunctionAndSetting functionAndSetting) { this.functionAndSetting = functionAndSetting; loget = new Loget(ref functionAndSetting); }
                public UploadFile(ref FunctionAndSetting functionAndSetting) { this.functionAndSetting = functionAndSetting; loget = new Loget(ref functionAndSetting); }
                ...
                public ResulUploadFileInfo UploadFileHendler(string PatchFile, byte MaxFilesUpload = 1, string Password = null, FunctionAndSetting.Massenge massenge = null)
                {
                    ...
                    else
                    {
                        ...
                        if (loget.SearchName(Path.GetFileName(PatchFile), FunctionAndSetting.TimeLiveFilesLog, PatchFile + FunctionAndSetting.ExpansionUpload + "Password" + Password))
                        {
                        ...
                        {
                            loget.GenereteName(Path.GetFileName(PatchFile), FunctionAndSetting.TimeLiveFilesLog, PatchFile + FunctionAndSetting.ExpansionUpload + "Password" + Password);
                        ...
                        while (file.Position < file.Length - 1)
                        {
                            ...
                            DataSaveInfo dataSaveInfo = new DataSaveInfo { FileIdName = ++c, Password = functionAndSetting.GenerateRandomString(10) };
            ...
            /// <summary>
            /// Информация о загруженном одном файле (некая бд)
            /// </summary>
        ...
        public class UploadDirectory
        {
            ...
            //==================================================== Инцилизация
            public UploadDirectory(Loget loget) { this.loget = loget; functionAndSetting = loget.function; }
            public UploadDirectory(FunctionAndSetting functionAndSetting, Loget loget) { this.functionAndSetting = functionAndSetting; this.loget = loget; this.loget.function = this.functionAndSetting; }
            public UploadDirectory(ref FunctionAndSetting functionAndSetting, ref Loget loget) { this.functionAndSetting = functionAndSetting; loget = new Loget(functionAndSetting); }
            public UploadDirectory(FunctionAndSetting functionAndSetting) { this.functionAndSetting = functionAndSetting; loget = new Loget(functionAndSetting); }
            public UploadDirectory(ref FunctionAndSetting functionAndSetting) { this.functionAndSetting = functionAndSetting; loget = new Loget(functionAndSetting); }
            public UploadDirectory() { loget = new Loget(functionAndSetting); }
            //===================================================== Методы
            ...
            {
                BdDirectory Bd = new BdDirectory { elementUploadDirs = new List<ElementUploadDir>(0) };
                UploadDownloadFile.UploadFile uploadFile = new UploadDownloadFile.UploadFile(ref functionAndSetting);
                var Folders = GetFolders(Patch);
                var Files = Get_elemSaves(GetFiles(Patch));
                if (loget.SearchName(Path.GetFileName(Patch) + FunctionAndSetting.ExpansionUploadDirectory, FunctionAndSetting.TimeLiveFilesLog, "Password" + Password + Patch))
                {
                    loget.Delite();
                }
                else
                {
                    loget.GenereteName(Path.GetFileName(Patch) + FunctionAndSetting.ExpansionUploadDirectory, FunctionAndSetting.TimeLiveFilesLog, "Password" + Password + Patch);
                    loget.AddData(JsonConvert.SerializeObject(Files), "SettingFiles");
                    loget.AddData(JsonConvert.SerializeObject(Folders), "SettingFolders");
                    foreach (var elem in Files)
                    {
                        List<string> NameFiles = new List<string>(0);
                        List<string> SaveDirectory = new List<string>(0);
                        List<long> Size = new List<long>(0);

                        foreach (var elemT in elem)
                        {
                            NameFiles.Add(Path.GetFileName(elemT));
                            SaveDirectory.Add(Path.GetFullPath(elemT).Replace(Patch, null));
                            Size.Add(GetLenFile(elemT));
                        }

                        ElementUploadDir element = new ElementUploadDir
                        {
                            NameFiles = NameFiles.ToArray(),
                            SaveDirectory = SaveDirectory.ToArray(),
                            Size = Size.ToArray(),
                            PasswordFile = functionAndSetting.GenerateRandomString(10)
                        }; NameFiles = null; SaveDirectory = null; Size = null;

                        if (elem.Length == 1)
                        {
                            element.resulUploadFileInfo = uploadFile.UploadFileHendler(elem[0], MaxFilesUpload, element.PasswordFile, massenge);
                            Bd.elementUploadDirs.Add(element);
                        }
                        else
                        {
                            List<byte[]> DatFile = new List<byte[]>(0);
                            foreach (var elemT in elem) DatFile.Add(File.ReadAllBytes(elemT));
                            string PatchS = Path.Combine(loget.GetUrlSaveData(), functionAndSetting.GenerateRandomString(10));
                            Directory.CreateDirectory(PatchS); PatchS = Path.Combine(PatchS, "MultiFiles.f");
                            File.WriteAllText(PatchS, JsonConvert.SerializeObject(DatFile.ToArray())); DatFile = null;
                            element.resulUploadFileInfo = uploadFile.UploadFileHendler(PatchS, MaxFilesUpload, element.PasswordFile, massenge);
                            Bd.elementUploadDirs.Add(element);
                            try { File.Delete(PatchS); } catch { }
                        }
                    }
                    Console.WriteLine(JsonConvert.SerializeObject(Bd));
                }
                return null;
            }
            /// <summary>
            ...
            /// <returns></returns>
            private List<string> GetFiles(string Patch)
            {
                if (Directory.Exists(Patch))
                {
                    ...
                    foreach (var Elem in all) Dat.AddRange(GetFiles(Elem));
                    return Dat;
                }
            ...
            /// <param name="Patch"></param>
            /// <returns></returns>
            private List<string> GetFolders(string Patch)
            {
                if (Directory.Exists(Patch))
                {
                    List<string> Dat = new List<string>();
                    // Получение список папок
                    string[] all = Directory.GetDirectories(Patch);
                    if (all.Length <= 0) return new List<string> { Patch };
                    foreach (var Elem in all) Dat.AddRange(GetFolders(Elem));
                    return Dat;
                }
                return null;
            }
            /// <summary>
            /// Метод придумывает как обьеденить файлы
            /// </summary>
            /// <param name="FilesFolders"></param>
            /// <returns></returns>
            private List<string[]> Get_elemSaves(List<string> FilesFolders)
            {
                List<string[]> elemSaves = new List<string[]>(0);
                List<string> ElemMinLen = new List<string>(0);
                foreach (var elem in FilesFolders)
                {
                    long len = GetLenFile(elem);
                    if (len > FunctionAndSetting.OneMb * functionAndSetting.MaxMbPart)
                        elemSaves.Add(new string[] { elem });
                    else
                        ElemMinLen.Add(elem);
                }
                FilesFolders = null;
                while (ElemMinLen.Count > 0)
                {
                    string elem = ElemMinLen[0];
                    ElemMinLen.Remove(elem);
                    long Size = GetLenFile(elem);
                    string retFile = null;
                    List<string> Files = new List<string> { elem };
                    while (true)
                    {
                        retFile = SearchFileOst(ElemMinLen.ToArray(), (FunctionAndSetting.OneMb * functionAndSetting.MaxMbPart - (long)((FunctionAndSetting.OneMb * functionAndSetting.MaxMbPart)*0.36f)) - Size);
                        if (retFile == null) break;
                        Size += GetLenFile(retFile);
                        ElemMinLen.Remove(retFile);
                        Files.Add(retFile);
                    }
                    elemSaves.Add(Files.ToArray());
                }
                return elemSaves;
            }
            /// <summary>
            /// Получение файла (максимального размера) который вписывается в указанный размер
            /// </summary>
            /// <param name="Files">Список в котором производится поиск</param>
            /// <param name="SearchMaxLen">Максимальный размер файла</param>
            /// <returns></returns>
            private static string SearchFileOst(string[] Files, long SearchMaxLen)
            {
                if (SearchMaxLen <= 0) return null;
                string PatchFile = null;
                long LenFile = long.MinValue;
                foreach (var elem in Files)
                {
                    long len = GetLenFile(elem);
                    if (len <= SearchMaxLen && len > LenFile)
                    {
                        LenFile = len;
                        PatchFile = elem;
                    }
            ...
            /// </summary>
            /// <param name="PatchFile">Путь к файлу</param>
            /// <returns></returns>
            private static long GetLenFile(string PatchFile)
            {
                try
                {
                    System.IO.FileInfo file = new System.IO.FileInfo(PatchFile);
                    return file.Length;
                }
                catch { return 0; }
            }
        ...
        //========================================================================= Структуры
        /// <summary>
        /// Один элемент бд по загрузке директорий
        /// </summary>
        public struct ElementUploadDir
        {
            /// <summary>
            /// Пароль на файл
            /// </summary>
            ...
            /// Ссылка на файл
            /// </summary>
            public UploadDownloadFile.ResulUploadFileInfo resulUploadFileInfo;
            /// <summary>
            /// Перечень относительный путей до файлов содержащиеся в данном файле
            /// </summary>
            public string[] SaveDirectory;
            /// <summary>
            /// Перечень названий файлов содержащиеся в данном файле
            /// </summary>
            public string[] NameFiles;
            /// <summary>
            /// Перечень размера файлов содержащиеся в данном файле
            /// </summary>
            public long[] Size;
        }
        /// <summary>
        ...
        {
            public List<ElementUploadDir> elementUploadDirs;
        }
    }
}